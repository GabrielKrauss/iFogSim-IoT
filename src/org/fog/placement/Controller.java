...
public class Controller extends SimEntity{
	...
	private void printEnergyConsumed() {
		long etotal = 0;
		long eiot = 0;
		long egat = 0;
		long eprox = 0;
		...
		for(FogDevice device : fogDevices){
			etotal += device.getEnergyConsumption();
			if(device.getName().startsWith("m")){
				eiot += device.getEnergyConsumption();
			}
			if(device.getName().startsWith("g")){
				egat += device.getEnergyConsumption();
			}
			if(device.getName().startsWith("p")){
				eprox += device.getEnergyConsumption();
			}
			
		}
		
		System.out.println("Energia iot usada: "+ eiot);
		System.out.println("Energia gateway usada: "+ egat);
		System.out.println("Energia proxy usada: "+ eprox);
		System.out.println("Energia total usada: "+ etotal);
	}
