...
public class CardiovascularHealthMonitoringApplication {
    ...
    private static void createFogDevices(int userId, Application app) throws NumberFormatException, IOException {
        ...
        if (locator.getLevelWiseResources(locator.getLevelID("Cloud")).size() == 1) {
            ...
            FogDevice cloud = createFogDevice("cloud", 44800, 16000, 100, 10000, 0.01, 1717.42, 1334.93, MicroserviceFogDevice.CLOUD); // creates the fog device Cloud at the apex of the hierarchy with level=0
            ...
            System.out.println("Locator def");
            System.out.println(locator.getLevelWiseResources(locator.getLevelID("Proxy")).size());
            for (int i = 0; i < locator.getLevelWiseResources(locator.getLevelID("Proxy")).size(); i++) {

                FogDevice proxy = createFogDevice("proxy-server_" + i, 2800, 16000, 10, 20, 0.0, 107.339, 83.4333, MicroserviceFogDevice.FON); // creates the fog device Proxy Server (level=1)
            ...
            for (int i = 0; i < locator.getLevelWiseResources(locator.getLevelID("Gateway")).size(); i++) {
                ...
                FogDevice gateway = createFogDevice("gateway_" + i, 2800, 8000, 50, 100, 0.0, 107.339, 83.4333, MicroserviceFogDevice.FCN);
    ...
    private static FogDevice addMobile(String name, int userId, Application app, int parentId) {
        FogDevice mobile = createFogDevice(name, 500, 1000, 100, 200, 0, 87.53, 82.44, MicroserviceFogDevice.CLIENT);
